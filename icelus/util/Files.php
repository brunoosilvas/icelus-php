...
class Files 
{
	...
	{
	    return file_exists($path . ($extension == null ? self::EXTENSION_DEFAULT : $extension));
	}
	...
	{
		$path = Utils::rootDir() . $file . self::EXTENSION_XML;
		if (!file_exists($path))			
			throw new \ErrorException(sprintf("XML notfound in directory '%s'", $pah));
